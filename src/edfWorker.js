...
function parseSignalSeries(buffer, header, signalIndex) {
  const view = new DataView(buffer);
  const values = [];
...
function parseSignals(buffer, header, mode) {
...
function crc16CcittFalse(bytes, start, end) {
  let crc = 0xffff;
...
function validateRecordCrc(buffer, header) {
...
async function parseEdfEntry(entry, mode, crcSidecar) {
...
function crcPathForEdf(relativePath) {
  return relativePath.replace(/\.edf$/i, ".crc");
}

async function buildCrcSidecars(entries) {
  const map = new Map();
